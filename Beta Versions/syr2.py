...
def checked(syr_dict, address):
    # Triggers when syringe box is checked or unchecked on the GUI.
    if syr_dict['syr_on'].isChecked():  # if box was checked
        # opens serial line at COM = syr_address
        if syr_dict['syr_chain'] is None:
            syr_dict['syr_chain'] = pumpy.Chain(str(syr_dict['syr_address'].text()))
        # creates syringe instance with address
        if address == 1: 
            syr_dict['syr_pump'] = pumpy.PHD2000(syr_dict['syr_chain'], address)
            syr_dict['infuse_on'].setEnabled(True)
        if address == 2:
            syr_dict['syr_pump_2'] = pumpy.PHD2000(syr_dict['syr_chain'], address)
            syr_dict['infuse_on_2'].setEnabled(True)
        syr_dict['output_box'].append(
                    'syringe pump %d connected' %address)
        syr_dict['run_syr_seq'].setEnabled(True)
        '''   
        except NameError:
            syr_dict['output_box'].append(
                    'Syringe pump connection failed')
        '''
    if not syr_dict['syr_on'].isChecked() and not syr_dict['syr_on_2'].isChecked:  # if box was unchecked
        if syr_dict['syr_pump']  is not None: syr_dict['syr_pump'].stop()
        if syr_dict['syr_chain'] is not None: syr_dict['syr_chain'].close()
        syr_dict['syr_seq_running'] = False
        syr_dict['infuse_on'].setEnabled(False)
        syr_dict['syr_flow_display'].setText('--')
        syr_dict['output_box'].append('syringe pump serial chain closed')
...
def infuse_2(syr_dict):

    if syr_dict['syr_pump_2'] is not None:        
        syr_dict['syr_pump_2'].infuse()
        syr_dict['infusing_2'] = True
    else: print('pump 2 not connected')
...
def stop_infusing_2(syr_dict):
    if syr_dict['syr_pump_2'] is not None:
        syr_dict['syr_pump_2'].stop()
        syr_dict['infusing_2'] = False
    else: print('pump 2 not connected')
